...
export default function Reset(props) {
	...
	useEffect(() => {
		(async () => {
			const body = { reset_password_key: props.match.params.reset_password_key }
	...
	const handleResetPassword = async () => {
		if(values.password === values.confirmPassword) {
			...
			const response = await PATCH(`/users/${user.username}/set_password`, body);
		...
		<div className={classes.root}>
			...
			<FormControl fullWidth className={clsx(classes.margin, classes.textField)}>
				...
				<FormHelperText id="change-password-helper-text">
					The password must be at least 8 characters long, contain a number, asymbol, an uppercase and a lowercase letter.
				</FormHelperText>
			</FormControl>
