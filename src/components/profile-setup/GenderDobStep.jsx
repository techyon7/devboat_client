			...
			{ props => (
				<form onSubmit={props.handleSubmit}>
					...
					<Box display="flex" flexDirection="column">
						...
						<FormControl component="fieldset" className={classes.formControl} name="sex">
			        <RadioGroup aria-label="gender" name="sex" value={props.values.sex} onChange={props.handleChange}>
			          <FormControlLabel value="female" control={<Radio />} label="Female" />
			          <FormControlLabel value="male" control={<Radio />} label="Male" />
			          <FormControlLabel value="other" control={<Radio />} label="Other" />
			        </RadioGroup>
							{/* Additional gender information input (shows when "other" is selected) */}
							{ (props.values.sex === "other") ? <TextField multiline name="gender-info" label="Gender Information" className="" /> : null}
			      </FormControl>
					...
					<Box mt={15} width={1} display="flex" justifyContent="space-between">
						<Button variant="contained" className={clsx(classes.margin)} onClick={previousPage}>
							<Box px={8}>Back</Box>
						</Button>
						<Button type="submit" variant="contained" className={clsx(classes.margin + "btn btn-success")}>
							<Box px={8}>Done</Box>
						</Button>
					</Box>
				</form>
